...
export default function CommunityPanel({base,token,feedback,onError}) {
    ...
    return <div className="journey-stack">
        <h3>偏好星图</h3>
        <p className="journey-muted">中心是你。距离越近，公开偏好越相似；方向仅用于排布，用户彼此间的距离没有相似度含义。</p>
        <p className="journey-muted">双方都需至少 3 个不同的公开收藏地点，当前你已公开 {neighbors?.signal_count||0} 个。相似度按地点数量收缩，兴趣标签不计为独立地点证据。</p>
        {!neighbors?.neighbors.length?<p>暂时没有足够的共同偏好。将收藏地点加入公开收藏夹后，再来查看同好。</p>:<>
            <svg viewBox="-1.2 -1.2 2.4 2.4" className="journey-taste" role="img" aria-label="以我为中心的偏好星图">
                ...
                <circle r=".07" fill="#cc7854"/><text y=".17" textAnchor="middle" fontSize=".09" fill="currentColor">我</text>
                {neighbors.neighbors.map(p=><g key={p.id} role="button" tabIndex="0" aria-label={`查看 ${p.username} 的公开收藏夹`} onClick={()=>perform(async()=>setPerson(await api(`/users/${p.id}/collections`)))} onKeyDown={e=>{if(e.key==='Enter'||e.key===' ') {e.preventDefault();perform(async()=>setPerson(await api(`/users/${p.id}/collections`)));}}}>
                    <circle cx={p.x} cy={p.y} r=".045" fill="#529488"/><title>{p.username} · {Math.round(p.similarity*100)}% · {p.shared.join('、')}</title>
                </g>)}
            </svg>
            {neighbors.neighbors.map(p=><button className="journey-row" key={p.id} onClick={()=>perform(async()=>setPerson(await api(`/users/${p.id}/collections`)))}><strong>{p.username}</strong><span>相似度 {Math.round(p.similarity*100)}% · {p.confidence}</span><small>{p.shared.join(' · ')}</small></button>)}
        </>}
        ...
        <h3>我的收藏夹</h3><p className="journey-muted">新收藏夹默认私密。公开后，其中地点会用于同好匹配；私人日记不会进入同好画像。</p>
        ...
        <label className="journey-check"><input type="checkbox" checked={settings.discovery} onChange={e=>setSettings({...settings,discovery:e.target.checked})}/>允许其他用户在同好星图看到我的用户名、兴趣标签和公开收藏夹</label>
