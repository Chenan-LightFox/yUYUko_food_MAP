...
export async function makeCover(cover,style='奶油',caption='',mapBlob=null) {
    const theme=themes[style]||themes.奶油, canvas=document.createElement('canvas');canvas.width=1080;canvas.height=1440;
    ...
    if(mapBlob) {
        ...
        ctx.setLineDash([]);
        stops.forEach((s,i)=>{if(s.lng==null)return;const p=point(s);ctx.fillStyle=theme.line;ctx.beginPath();ctx.arc(...p,25,0,2*Math.PI);ctx.fill();ctx.fillStyle=theme.bg;ctx.font='bold 25px sans-serif';ctx.textAlign='center';ctx.fillText(String(i+1),p[0],p[1]+9);ctx.textAlign='left';});
